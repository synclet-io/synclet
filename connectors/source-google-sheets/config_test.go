...
func TestConfigUnmarshal(t *testing.T) {
	...
	t.Run("batch_size as string", func(t *testing.T) {
		data := `{
			"spreadsheet_id": "abc123",
	...
	t.Run("batch_size as number", func(t *testing.T) {
		data := `{
			"spreadsheet_id": "abc123",
			"credentials": {"auth_type": "Client"},
			"batch_size": 200
	...
	t.Run("batch_size default", func(t *testing.T) {
		data := `{
			"spreadsheet_id": "abc123",
			"credentials": {"auth_type": "Client"}
		}`
		...
		err := json.Unmarshal([]byte(data), &cfg)
		require.NoError(t, err)
		assert.Equal(t, 1000000, cfg.BatchSize)
	})
}
