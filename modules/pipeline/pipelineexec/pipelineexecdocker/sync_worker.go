...
// syncExecutor abstracts DockerSyncExecutor for testability.
type syncExecutor interface {
	Execute(ctx context.Context, bundle *pipelineexec.SyncBundle) (*pipelineservice.SyncStats, error)
}

// DockerSyncWorker is a jobber that polls for scheduled jobs, claims them,
// and runs sync execution using DockerSyncExecutor (which uses pipelineexec.Run
...
// polling, delegates to DockerSyncExecutor, then handles completion via ExecutorBackend.
func (w *DockerSyncWorker) executeJob(ctx context.Context, result *pipelinejobs.ClaimJobBundleResult) {
	...
	bundle := &pipelineexec.SyncBundle{
		Job:                   result.Job,
		...
		CustomNamespaceFormat: pipelineexec.NilIfEmpty(result.CustomNamespaceFormat),
		StreamPrefix:          pipelineexec.NilIfEmpty(result.StreamPrefix),
	}
	...
	if completeErr := w.backend.UpdateJobStatus(ctx, pipelineexec.UpdateJobStatusParams{
		JobID:        result.Job.ID,
		Success:      execErr == nil,
		ErrorMessage: errMsg,
...
func (w *DockerSyncWorker) failJob(ctx context.Context, jobID, workspaceID, connectionID uuid.UUID, reason error) {
	...
	if err := w.backend.UpdateJobStatus(ctx, pipelineexec.UpdateJobStatusParams{
		JobID:        jobID,
		Success:      false,
