...
type ConnectorClient struct {
	runner pkgcontainer.Runner
}

// NewConnectorClient creates a new ConnectorClient backed by the given container runner.
func NewConnectorClient(runner pkgcontainer.Runner) *ConnectorClient {
	return &ConnectorClient{runner: runner}
}

// ResourceLimits holds memory and CPU limits for a container.
...
func (c *ConnectorClient) Spec(ctx context.Context, image string) (*protocol.ConnectorSpecification, error) {
	messages, err := c.runAndCollect(ctx, pkgcontainer.RunOptions{
		Image:       image,
		Command:     []string{"spec"},
		NetworkMode: "none",
	})
...
func (c *ConnectorClient) Read(ctx context.Context, image string, config json.RawMessage, catalog *protocol.ConfiguredAirbyteCatalog, state json.RawMessage, labels map[string]string, limits ResourceLimits) (io.ReadCloser, func(), error) {
	...
	result, err := c.runWithAutoPull(ctx, pkgcontainer.RunOptions{
		Image:       image,
		Command:     cmd,
		ConfigFile:  config,
...
func (c *ConnectorClient) Write(ctx context.Context, image string, config json.RawMessage, catalog *protocol.ConfiguredAirbyteCatalog, stdin io.Reader, labels map[string]string, limits ResourceLimits) (io.ReadCloser, func(), error) {
	catalogBytes, err := json.Marshal(catalog)
	if err != nil {
...
func (r *exitAwareReader) Read(p []byte) (int, error) {
	...
	if err == io.EOF {
		...
		if *r.exitCode != 0 {
			...
			return n, &pipelineservice.ExitCodeError{
				ExitCode: *r.exitCode,
				Role:     r.role,
				Stderr:   stderrStr,
			}
...
func (c *ConnectorClient) runWithAutoPull(ctx context.Context, opts pkgcontainer.RunOptions) (*pkgcontainer.RunResult, error) {
...
func (c *ConnectorClient) runAndCollect(ctx context.Context, opts pkgcontainer.RunOptions) ([]*protocol.AirbyteMessage, error) {
