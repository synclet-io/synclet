...
func TestLaunchConnectorTask_CreatesSecret(t *testing.T) {
	...
	// Verify coordinator args do NOT contain --task-config (config is in Secret, mounted on connector).
	job, err := client.BatchV1().Jobs("test-ns").Get(context.Background(), jobName, metav1.GetOptions{})
	require.NoError(t, err)
	...
	// Verify the connector container has a SubPath mount from the Secret.
	connectorContainer := job.Spec.Template.Spec.Containers[1]
	var configMount *corev1.VolumeMount

	for i := range connectorContainer.VolumeMounts {
		if connectorContainer.VolumeMounts[i].Name == "task-configs" {
			configMount = &connectorContainer.VolumeMounts[i]

			break
		}
	}

	require.NotNil(t, configMount, "connector must have task-configs mount")
	assert.Equal(t, "/shared/config.json", configMount.MountPath)
	assert.Equal(t, "task-config", configMount.SubPath)
