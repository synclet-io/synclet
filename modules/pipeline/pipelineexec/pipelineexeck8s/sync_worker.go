...
// K8sSyncLauncher abstracts the K8s runner for launching sync jobs.
type K8sSyncLauncher interface {
	LaunchSyncJob(ctx context.Context, opts K8sSyncJobOptions) (string, error)
}
...
type K8sSyncJobOptions struct {
	...
	RuntimeConfig     string // JSON runtime config for source resource limits
	DestRuntimeConfig string // JSON runtime config for destination resource limits
...
func (w *K8sSyncWorker) Execute(ctx context.Context) error {
	...
	pipelinecatalog.ApplyNamespaceAndPrefix(destCatalog, result.NamespaceDefinition, pipelineexec.NilIfEmpty(result.CustomNamespaceFormat), pipelineexec.NilIfEmpty(result.StreamPrefix))
	...
	k8sJobName, err := w.k8sRunner.LaunchSyncJob(ctx, opts)
	if err != nil {
