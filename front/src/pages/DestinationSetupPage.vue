...
import { createDestination, testConnection as testDestinationConnection } from '@entities/destination/api'
...
onMounted(async () => {
  try {
    ...
    connectors.value = all.filter(c => c.connectorType === 'destination')
  }
  catch (e: unknown) {
...
async function handleTestConnection() {
  ...
  error.value = ''
  try {
    const { taskId } = await testDestinationConnection({
      managedConnectorId: selectedConnector.value.id,
      config: cleanConfig(configValues.value),
    })
...
<template>
  <PageHeader title="Add Destination" description="Configure a new data destination" />
  ...
  <div v-if="step === 1">
    <SInput v-model="search" placeholder="Search destination connectors..." class="mb-4" />
        ...
        <form class="space-y-4" @submit.prevent="handleCreateDestination">
          ...
          <div class="flex gap-3 pt-2">
            ...
            <SButton type="submit" :loading="loading" :disabled="!name">
              {{ loading ? 'Testing connection...' : 'Create Destination' }}
            </SButton>
          </div>
        </form>
      </SCard>
